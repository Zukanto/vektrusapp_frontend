...
import { TrendingUp, MessageCircle, Target, Building2, Briefcase, Smile, Sparkles, Laugh, Wand2 } from 'lucide-react';
...
const goals = [
  ...
  { id: 'leads', label: 'Leads & Verkäufe', subtitle: 'Anfragen und Conversions', icon: Target, color: '#F4BE9D' },
  { id: 'brand_awareness', label: 'Markenbekanntheit', subtitle: 'Branding und Wiedererkennung', icon: Building2, color: '#B6EBF7' }
];
...
const Step3GoalStyle: React.FC<Step3Props> = ({ data, onUpdate, brandProfile, onNavigateToBrandStudio, hasBrandDefaults }) => {
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      {onNavigateToBrandStudio !== undefined && (
        <BrandStyleIndicator
          brandProfile={brandProfile ?? null}
          imageGenerationEnabled={data.imageGeneration?.enabled ?? false}
      ...
      <div className="text-center">
        <h3 className="text-2xl font-bold text-[#111111] mb-2" style={{ fontFamily: 'Manrope, sans-serif' }}>
          Wie soll dein Content wirken?
        </h3>
        <p className="text-[#7A7A7A] leading-relaxed">
          Wähle das Ziel und den Stil deiner Posts.
        </p>
      </div>

      <div>
        <label className="block text-sm font-semibold text-[#111111] mb-3">Ziel</label>
